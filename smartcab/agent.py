...
class LearningAgent(Agent):
    ...
    def get_maxQ(self, state):
        ...
        actions_dict = self.Q[state]
        actions = [key for key,val in actions_dict.iteritems() if val == max(actions_dict.values())]
    ...
    def learn(self, state, action, reward):
        ...
        predict_value = self.Q[state][action]
        taget = reward
        # if state[0] is None:
        #     taget = reward + self.get_maxQ(state)[1]
        # taget = reward + self.get_maxQ(state)[1]
        self.Q[state][action] += self.alpha * (taget - predict_value)
